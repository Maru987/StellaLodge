...
export function Contact({ property, className }: ContactProps) {
  const { contactEmail, socialLinks, externalBookingPlatforms } = property;
          ...
          <div className="bg-gray-50 rounded-2xl p-8 shadow-sm">
            ...
            {socialLinks && Object.keys(socialLinks).length > 0 && (
              <div className="mb-8 text-center">
                <h4 className="font-medium mb-4 text-black">Suivez-nous</h4>
            ...
            {/* Plateformes de réservation externes */}
            {externalBookingPlatforms && Object.keys(externalBookingPlatforms).length > 0 && (
              <div className="text-center">
                <h4 className="font-medium mb-4 text-black">Réservez aussi sur</h4>
                
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
                  <div className="flex items-center justify-center mb-2">
                    <Clock className="w-5 h-5 text-amber-600 mr-2" />
                    <p className="font-medium text-amber-800">Bientôt disponible</p>
                  </div>
                  <p className="text-amber-700 text-sm">
                    Nos pages Airbnb et Booking.com sont en cours de création et seront disponibles prochainement.
                  </p>
                </div>
                
                <div className="flex flex-wrap gap-4 justify-center">
                  {Object.entries(externalBookingPlatforms)
                    .filter(([platform]) => platform !== 'airbnb' && platform !== 'bookingCom')
                    .map(([platform, url]) => (
                      url && (
                        <a 
                          key={platform}
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center text-gray-700 hover:text-blue-600 transition-colors bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm"
                        >
...
// Fonction pour obtenir le label des plateformes de réservation
